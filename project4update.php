...
<html>
    ...
    <body>
        ...
        <form id="updateform" method="POST" action="project4updateverify.php" onsubmit="confirm();">
            <p id="title"><b>House of Health: Update a Patient Record</b></p>
            <label for="shots" class="labels">Shots:</label>
            <label class="rqlabel">REQUIRED</label>
            <input class="text" type="text" name="shots" id="shots" placeholder="Example: Flu">
            <label for="illness" class="labels">Illness:</label>
            <label class="rqlabel">REQUIRED</label>
            <input class="text" type="text" name="illness" id="illness" placeholder="Example: Vertigo">
    ...
    <script type="text/javascript">
        function confirm() {
            if (!(/.*/.test(document.getElementById('shots').value))) {
                window.alert("Shots missing. Please enter.");
            }
            if (!(/.*/.test(document.getElementById('illness').value))) {
                window.alert("Illness missing. Please enter.");
            }
            if (!(/[0-9]+/.test(document.getElementById('patientid').value))) {
                window.alert("Patient ID missing. Please enter.");
            }
        }
    </script>
</html>
