...
<html>
    ...
    <body>
        ...
        <form id="bookproform" method="POST" action="project4bookproverify.php" onsubmit="confirm();">
            <p id="title"><b>House of Health: Book a Patient's Procedure</b></p>
            <label for="prodate" class="labels">Procedure Date:</label>
            <label class="rqlabel">REQUIRED</label>
            <input class="text" type="text" name="prodate" id="prodate" placeholder="Example: 6/21/2023">
            <label for="protype" class="labels">Procedure Type:</label>
            <label class="rqlabel">REQUIRED</label>
            <input class="text" type="text" name="protype" id="protype" placeholder="Example: Endoscopy">
            <label for="appid" class="labels">Appointment ID: </label>
            <label class="rqlabel">REQUIRED</label>
            <input class="text" type="text" name="appid" id="appid" placeholder="Example: 341">
            <button class="buttons" type="submit" name="submit" id="submit">Submit</button><br>
        </form>
    </body>
    <script type="text/javascript">
        function confirm() {
            if (!(/.+/.test(document.getElementById('prodate').value))) {
                window.alert("Procedure date missing. Please enter.");
            }
            if (!(/.+/.test(document.getElementById('protype').value))) {
                window.alert("Procedure type missing. Please enter.");
            }
