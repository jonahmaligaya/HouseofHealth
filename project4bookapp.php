...
<html>
    ...
    <body>
        ...
        <form id="bookappform" method="POST" action="project4bookappverify.php" onsubmit="confirm();">
            <p id="title"><b>House of Health: Book a Patient's Appointment</b></p>
            <label for="appdate" class="labels">Appointment Date:</label>
            <label class="rqlabel">REQUIRED</label>
            <input class="text" type="text" name="appdate" id="appdate" placeholder="Example: 4/27/2023">
            <label for="apptype" class="labels">Appointment Type:</label>
            <label class="rqlabel">REQUIRED</label>
            <input class="text" type="text" name="apptype" id="apptype" placeholder="Example: Well Visit">
            <label for="doctorid" class="labels">Doctor ID: </label>
            <label class="rqlabel">REQUIRED</label>
            <input class="text" type="text" name="doctorid" id="doctorid" placeholder="Example: 3">
            <button class="buttons" type="submit" name="submit" id="submit">Submit</button><br>
        </form>
    </body>
    <script type="text/javascript">
        function confirm() {
            if (!(/.+/.test(document.getElementById('appdate').value))) {
                window.alert("Appointment date missing. Please enter.");
            }
            if (!(/.+/.test(document.getElementById('apptype').value))) {
                window.alert("Appointment type missing. Please enter.");
            }
            if (!(/[0-9]+/.test(document.getElementById('doctorid').value))) {
                window.alert("Doctor ID missing. Please enter.");
            }
        }
    </script>
</html>
