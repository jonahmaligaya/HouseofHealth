...
<html>
    ...
    <body>
        ...
        <form id="bookproform" method="POST" action="project4createverify.php" onsubmit="confirm();">
            <p id="title"><b>House of Health: Create a New Patient Account</b></p>
            <label for="patientfname" class="labels">Patient's First Name: </label>
            <label class="rqlabel">REQUIRED</label>
            <input class="text" type="text" name="patientfname" id="patientfname" placeholder="Example: Josie">
            <label for="patientlname" class="labels">Patient's Last Name: </label>
            <label class="rqlabel">REQUIRED</label>
            <input class="text" type="text" name="patientlname" id="patientlname" placeholder="Example: Smith">
            <label for="patientid" class="labels">Patient ID: </label>
            <label class="rqlabel">REQUIRED</label>
            <input class="text" type="text" name="patientid" id="patientid" placeholder="Example: 320">
            <button class="buttons" type="submit" name="submit" id="submit">Submit</button><br>
        </form>
    </body>
    <script type="text/javascript">
        function confirm() {
            if (!(/[A-Za-z]+/.test(document.getElementById('patientfname').value))) {
                window.alert("Patient's Last Name. Please enter.");
            }
            if (!(/[A-Za-z]+/.test(document.getElementById('patientlname').value))) {
                window.alert("Patient's Last Name missing. Please enter.");
            }
            if (!(/[0-9]+/.test(document.getElementById('patientid').value))) {
